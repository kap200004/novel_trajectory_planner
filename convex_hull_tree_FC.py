...
class node:
    ...
    def build_node(self, points, start, end, leaf_pointers):
        if start == end:
            ...
            left_upper_catalog = self.left_child.upper_catalog
            right_upper_catalog = self.right_child.upper_catalog
            my_cat_pointer = 0
            left_cat_pointer = 0
            right_cat_pointer = 0

            new_ucat = []
            while not (my_cat_pointer == len(self.upper_catalog) and left_cat_pointer == len(left_upper_catalog) and right_cat_pointer == len(right_upper_catalog)):
                cur_min = np.min([self.upper_catalog[my_cat_pointer], left_upper_catalog[left_cat_pointer], right_upper_catalog[right_cat_pointer]])
                new_ucat.append(cur_min)
                if self.upper_catalog[my_cat_pointer] == cur_min:
                    my_cat_pointer += 1
                if left_upper_catalog[left_cat_pointer] == cur_min:
                    left_cat_pointer += 1
                if right_upper_catalog[right_cat_pointer] == cur_min:
                    right_cat_pointer += 1


            for i, thing in enumerate(new_ucat):
                if i < len(new_ucat) - 1:
                    if thing > new_ucat[i + 1]:
                        print('This is out of order')
                        print(thing)
                        print(new_ucat[i + 1])
                        print(self.upper_catalog)
                        print(left_upper_catalog)
                        print(right_upper_catalog)
                        break
            self.upper_catalog = new_ucat
...
def convex_hull(points):
    ...
    Implements Andrew's monotone chain algorithm. O(n) complexity.
    """
